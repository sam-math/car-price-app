...
# Load the performance metrics and modeling results
performance_df = pd.read_csv('06_app/app_files/performance_metrics.csv')

# Convert all results into a single DataFrame
results_df = pd.read_csv('06_app/app_files/final_results_df.csv')

car_pictures_df = pd.read_csv('06_app/app_files/car_pictures_table_all.csv')
...
with st.sidebar:
        ...
        default_brand = 'peugeot'
        brands_list = performance_df['brand'].unique().tolist()
        default_brand_index = brands_list.index(default_brand)    
        selected_brand = st.selectbox(label='Select Brand',
                              options= brands_list,
                              index=default_brand_index,
        ...
        brand_models_list = performance_df[performance_df['brand'] == selected_brand]['model'].unique().tolist()
        if selected_brand == default_brand:
                default_model = '3008'
                default_model_index = brand_models_list.index(default_model)
        else:
        ...
        selected_transmission = st.multiselect(label='Select Transmission(s)',
                                               options=['manual', 'automatic'],
                                               default=['manual', 'automatic'])
        if len(selected_transmission) == 2:
                selected_transmission = [0,1]
        elif 'automatic' in selected_transmission:
               selected_transmission = [1]
        elif 'manual' in selected_transmission:
                selected_transmission = [0]      
        else:
        ...
        performance = performance_df[(performance_df['brand'] == selected_brand) & (performance_df['model'] == selected_model)]
        st.write("Performance Metrics:")
        st.write(performance)

        if selected_brand and selected_model:
                try:
                        author_list = ['alex', 'rudolf','thomas', 'vauxford', 'dinkun']
                        car_pictures_brand_model = car_pictures_df[(car_pictures_df['author'] != 'Unknown') & 
                                        (car_pictures_df['brand'] == selected_brand) &
                                        (car_pictures_df['model'] == selected_model)
                                         & (car_pictures_df['author'].str.lower().str.contains('|'.join(author_list)))
                                        ].reset_index(drop=True)                                       

                        selected_picture = car_pictures_brand_model.iloc[[0],:]

                        image_author = selected_picture['author'][0]
                        image_full_url = selected_picture['fullurl'][0]
                        image_picture_url = selected_picture['imageinfo.0.thumburl'][0]
                        st.image(image_picture_url, use_column_width=True)
                        st.info(f"image by: [{image_author}]({image_full_url}), [CC BY-SA 4.0](https://creativecommons.org/licenses/by-sa/4.0) via Wikimedia Commons")
                except:
                        st.info('Car image not available')
...
if selected_model and selected_transmission:
        # Filter results for the selected brand-model
        selected_car = results_df[
          (results_df['brand'] == selected_brand) & 
          (results_df['model'] == selected_model) &
          (results_df['is_automatic'].isin(selected_transmission)) &
          (results_df['km'].between(selected_km[0],selected_km[1])) &
          (results_df['age_years'].between(selected_age[0],selected_age[1]))]
        ...
        with left_col:
                ...
                # Ensure X has data before applying polynomial features
                if X.shape[0] > 0:
                        poly = PolynomialFeatures(degree=3)
                        X_poly = poly.fit_transform(X)
                        ...
                        # Create a Plotly scatter plot with alpha
                        fig = px.scatter(
                        selected_car, 
                        x='km', 
                        y='price', 
                        color='is_automatic',
                        opacity=0.5,  # Set the opacity for the scatter plot markers
                        labels={'km': 'Mileage (km)', 'price': 'Price'},
                        title=f'Price vs Mileage (km) for {selected_brand} {selected_model}',
                        color_discrete_map={1: '#002776' , 0: '#92d400'}
                        )

                        # Add the polynomial regression line with alpha
                        fig.add_scatter(
                        x=X_range.flatten(), 
                        y=y_pred, 
                        mode='lines', 
                        ...
                        opacity=0.7  # Set the opacity for the regression line
                        )
        ...
        with right_col: 
                ...
                if X.shape[0] > 0:
                        ...
                        # Create a Plotly scatter plot with custom colors for 'is_automatic'
                        fig_age = px.scatter(
                        selected_car, 
                        x='age_years', 
                        y='price', 
                        color='is_automatic',
                        opacity=0.3,  # Set the opacity for the scatter plot markers
                        labels={'age_years': 'Car Age (years)', 'price': 'Price'},
                        title=f'Price vs Car Age (years) for {selected_brand} {selected_model}',
                        color_discrete_map={1: '#002776' , 0: '#92d400'}  # Map 'manual' (0) to blue and 'automatic' (1) to green
                        )

                        # Add the polynomial regression line for age with alpha
                        fig_age.add_scatter(
                        x=X_age_range.flatten(), 
                        y=y_age_pred, 
                        mode='lines', 
                        showlegend=False,
                        # name='Polynomial Regression', 
                        line=dict(color='#008080', width=3),
                        opacity=0.8  # Set the opacity for the regression line
                        )
        ...
        # Display prediction results (already ordered)
        st.write("Prediction Results (with all columns):")
        edited_df = st.data_editor(selected_car, use_container_width=True)